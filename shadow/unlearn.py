...
def blast_radius(client: str, track: str, rule_ids: set[str], precedent_id: str | None, version: int) -> dict:
    ...
    metas = sorted((json.loads(p.read_text()) for p in db.RUNS.glob("*/run.json")), key=lambda m: (m["created_at"], m.get("playbook_version") or 0, m["run_id"]))   # "<run>__after_<id>" sorts after its base run within the same second
    for meta in metas:
        ...
        for line in (db.RUNS / meta["run_id"] / "resolutions.jsonl").read_text().splitlines():
            it = json.loads(line)
            for lid in it["resolution"].get("ledger_ids", []):
                standing.pop((meta["period"], lid), None)
    ...
    for period in sorted({p for p, _ in touched}):
        ...
        pipeline.run(client, period, "corrected", track, version=version, use_llm=False, run_id=rid,
